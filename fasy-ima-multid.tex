\documentclass{beamer}
\mode<presentation>
%% \mode<handout>{\setbeamercolor{background canvas}{bg=black!5}}
\usetheme{CambridgeUS}
\usecolortheme{dolphin}

\usepackage{bstyle}

\title{Statistics for a Computational Topologist}
\subtitle{Day 1}
\author[B.\ Fasy]{Brittany Terese Fasy\\ TA: Samuel A.\ Micka}
\institute[MSU]{School of Computing and Dept.\ of Mathematical Sciences\\ Montana State University}
\date{\today}

\begin{document}

\begin{frame}
\titlepage
\end{frame}

\input{body/intro}
\input{body/descriptors}
\input{body/pd-conf}
\input{body/distances}
\input{body/hw}

%% Day 2
\begin{frame}
\subtitle{Day 2}
\titlepage
\end{frame}

\frame{
\begin{itemize}
    \item Talk about functional summaries
    \item Pair/Share with the HW exercise.
    \item Conclude with an example: QuBBD?
    \item end with a probability problem they can think about? perhaps
        probability of capturing a vertex from a random set of directions?
        \todo{Sam, would you be able to create a small example that we've fully
        worked out?  This could be a nice plug for our CCCG work.}
\end{itemize}
}

\end{document}
